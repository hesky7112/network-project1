...
export default function UserManagement() {
  ...
  const fetchRoles = async () => {
    try {
      const resp = await apiClient.getPublicRoles();
      setAvailableRoles(resp);
      if (!newUser.role && resp.length > 0) {
        setNewUser(prev => ({ ...prev, role: resp[resp.length - 1].name }));
      }
    } catch (err) {
  ...
  const handleSaveUser = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUser) {
        ...
        toast.success("New user added to system");
      }
      setIsModalOpen(false);
      setEditingUser(null);
      setNewUser({ username: '', email: '', password: '', role: availableRoles[availableRoles.length - 1]?.name || 'Viewer' });
      fetchUsers();
    } catch (err) {
      toast.error(editingUser ? "Update failed" : "Creation failed");
      console.error(err);
    }
  };
  ...
  const columns: ColumnDef<Operator>[] = [
    {
      ...
      header: () => (
        <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Username</span>
      ),
      cell: ({ row }) => (
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const displayRole = role === 'admin' || role === 'Super Admin' ? 'Administrator' :
          role === 'engineer' || role === 'Network Admin' ? 'Engineer' :
            role === 'auditor' || role === 'Technician' ? 'Auditor' :
              role === 'viewer' || role === 'Viewer' ? 'Viewer' : role;

        return (
          <span className={cn(
            "px-2 py-0.5 border text-[8px] font-black uppercase tracking-widest",
            role === 'admin' || role === 'Super Admin' || role === 'Administrator' ? "border-earth-green/30 bg-earth-green/10 text-earth-green" :
              role === 'engineer' || role === 'Network Admin' || role === 'Engineer' ? "border-blue-400/30 bg-blue-400/10 text-blue-400" :
                role === 'auditor' || role === 'Technician' || role === 'Auditor' ? "border-amber-500/30 bg-amber-500/10 text-amber-500" :
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] mt-2">Manage user access and roles</p>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between mt-8 pt-8 border-t border-white/5 gap-6">
            ...
            <div className="flex items-center gap-4 w-full sm:w-auto">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic">Status: <span className="text-white">{loading ? 'Loading...' : '100%'}</span></div>
              </div>
            </div>
          </div>
        </GlassWrapper>
        ...
        <GlassWrapper className="bg-[#0a0a0c] border-white/5 rounded-sm overflow-hidden p-0 min-h-[400px] flex flex-col items-center justify-center">
          {loading ? (
            <div className="flex flex-col items-center gap-4">
              ...
              <span className="text-[10px] font-black text-slate-600 uppercase tracking-[0.4em]">Loading users...</span>
            </div>
          ) : (
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            <div className="bg-[#0a0a0c] border border-white/10 w-full max-w-md p-8 rounded-sm relative overflow-hidden">
              ...
              <div className="mb-8">
                <h2 className="text-2xl font-black text-white uppercase tracking-tighter italic italic">{editingUser ? 'MANAGE USER' : 'ADD NEW USER'}</h2>
                <p className="text-[9px] font-bold text-slate-500 uppercase tracking-[0.3em] mt-1 italic">{editingUser ? 'Updating user settings' : 'Creating new user profile'}</p>
              </div>
              ...
              <form onSubmit={handleSaveUser} className="space-y-6">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic ml-1">Primary Email</label>
                  <input
                    required
                    type="email"
                    value={newUser.email}
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic ml-1">Password</label>
                  <input
                    required
                    type="password"
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic ml-1">Access Level</label>
                  <select
                    value={newUser.role}
                  ...
                  >
                    {availableRoles.map((r: any) => {
                      const display = r.name === 'Super Admin' ? 'ADMINISTRATOR' :
                        r.name === 'Network Admin' ? 'ENGINEER' :
                          r.name === 'Technician' ? 'AUDITOR' :
                            r.name === 'Viewer' ? 'VIEWER' : r.name.toUpperCase();
                      return (
                        <option key={r.id} value={r.name} className="bg-[#0a0a0c]">
                          Level {r.level} ({display})
                        </option>
                      );
                    })}
                  </select>
